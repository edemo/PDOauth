...
class UserInfoTest(PDUnitTest, UserUtil, CryptoTestUtil, AuthProviderUtil):
    ...
    def test_logged_in_user_can_get_its_info(self):
    ...
    def test_userid_returned_is_the_string_one(self):
        resp = self.showUserByCurrentUser('me')
        self.assertEqual(resp.status_code, 200)
        data = self.fromJson(resp)
        userid = data['userid']
        self.assertTrue(isinstance(userid,str))

    
    def test_user_info_contains_assurance(self):
    ...
    def test_user_info_contains_hash(self):
        current_user = self.controller.getCurrentUser()
        ...
        self.assertEqual(resp.status_code, 200)
        data = self.fromJson(resp)
        self.assertEqual(data['hash'],current_user.hash)
    ...
    def test_users_without_assurer_assurance_cannot_get_email_and_digest_for_anyone(self):
        current_user = self.controller.getCurrentUser()
        targetuser=self.createUserWithCredentials().user
        Assurance.new(targetuser,'test',current_user)
        target = User.getByEmail(self.userCreationEmail)
    ...
    def test_querying_nonexistent_user_gives_404(self):
        self._createAssurer()
        with self.assertRaises(ReportedError) as context:
    ...
    def test_user_id_shown_to_the_application_differs_from_the_user_id(self):
        userid = self.cred.user.userid
        userinfo = self.getUserInfo()
        self.assertTrue(userid != userinfo['userid'])

    
    def test_user_id_shown_to_the_application_does_not_change_over_time(self):
    ...
    def test_email_shown_to_the_application_does_not_change_over_time(self):
        tokens = self.obtainCodeAndCallTokenInterface()
        userinfo1 = self.showUserByServer(tokens)
        Assurance.new(self.cred.user, 'test', self.cred.user)
        userinfo2 = self.showUserByServer(tokens)
    ...
    def test_the_userid_shown_for_the_same_user_should_be_different_for_different_applications(self):
    ...
    def test_the_email_address_shown_for_the_same_user_should_be_different_for_different_applications(self):
        userinfo1 = self.getUserInfo()
        self.prepareGetUserInfo()
        userinfo2 = self.getUserInfo()
    ...
    def test_the_applications_do_not_receive_credential_data_from_the_user(self):
        userinfo = self.getUserInfo()
        self.assertTrue('credentials' not in userinfo)

    
    def test_the_applications_do_not_receive_hash_from_the_user(self):
        userinfo = self.getUserInfo()
        self.assertTrue('hash' not in userinfo)
    ...
    def test_the_applications_do_not_receive_anything_beyond_mapped_email_address__mapped_user_id_and_filtered_list_of_assurance_names(self):
        userinfo = self.getUserInfo()
        self.assertEqual(set(userinfo.keys()),set(['userid', 'assurances', 'email']))
